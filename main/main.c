...
#endif
// no sensitive data in this project, so no need for random address setting
static const char *tag = "BLEPRPH_wifiMQTT";
...
#endif
// doesn't need enhanced ATT protocol, saves storage by =0
...
void app_main(void)
{
    ...
    rc = ble_svc_gap_device_name_set("nimble-bleprph-ESP32S3");
    assert(rc == 0);
#endif
    ...
    //     return;
    // }
    // extra content, host task naturally uses on sync, and starts advertising

    /* Initialize command line interface to accept input from user */
    // read what are availabel command
    rc = scli_init();
    if (rc != ESP_OK)
    {
